...
class DataManagement(object):
    ...
    __REGION_L1 = ee.Geometry.Polygon(
        [[[-30, -40], [65, -30], [65, 40], [-30, 40]]])
    __REGION = [[-25.0, -37.0], [60.0, -41.0],
    ...
    def __init__(self, usr, pwd):
        ...
        self.dataManagement_logger.debug(
            'Scale used for Level 1 exports %f ' % DataManagement.__SCALE_CALC)
    ...
    def get_assets_info(self, id_name):
        ...
        """
        asset_list = ['AET', 'NPP', 'AGBP',
                      'RET', 'PCP', 'LCC', 'TFRAC', 'PHE']
        ...
        if ee.data.getInfo(self.asset_path):
            self.dataManagement_logger.debug(
                'Collection %s Exists' % self.asset_path)
        else:
            ee.data.createAsset(
                {'type': ee.data.ASSET_TYPE_IMAGE_COLL},  self.asset_path)
            self.dataManagement_logger.debug(
                'Collection %s Created' % self.asset_path)

        ids_items_in_assets = ee.data.getList(params={'id':  self.asset_path})
        names_items_in_assets = [os.path.basename(
            asset['id']) for asset in ids_items_in_assets]
    ...
    def data_management(self, active_session, upload_url, images_already_in_asset, file_to_be_loaded,
        ...
        if name_image_to_load_in_asset in images_already_in_asset:
            self.dataManagement_logger.error(
                "%s already in collection" % name_image_to_load_in_asset)
            already_uploaded = True

        if os.path.exists(file_to_be_loaded) and not already_uploaded:
            self.__upload_image(file_to_be_loaded, active_session,
                                upload_url, final_image_name_in_asset, metadata, no_data)
        else:
            self.dataManagement_logger.debug(
                '%s already uploaded in GEE - Deleting old file' % name_image_to_load_in_asset)
    ...
    def metadata_generator(gee_file, level, seasonal=False, area='AfNE'):
        ...
        if seasonal:
            seasons_list = ['s1', 's2']
            season_position = map(
                lambda season: id_no.find(season), seasons_list)[0]
            date_part = id_no.split('_')[-2]
            season_numeric = int(
                id_no[season_position + 1:season_position + 2])
            season_time = id_no.split('_')[-1]
            year = int('20' + date_part[0:2])
            area_part = id_no.split('_')[1]
            start_date = datetime.strptime(
                "01-01-%s 12:00:00" % str(year), "%d-%m-%Y %H:%M:%S")
            unix_time_manual = int(
                (start_date - initial_date).total_seconds()) * 1000
            ...
            if len(date_part) == 4:
                ...
                unix_time_manual = (
                    start_date - initial_date).total_seconds() * 1000
                metadata = {
                    'id_no': id_no, 'system:time_start': unix_time_manual, 'level': level, 'area': area}
    ...
    def running_tasks(tasks):
        ...
        tasks_running = [task for task in tasks if task.config['state'] in (
            u'RUNNING', u'UNSUBMITTED', u'READY')]
    ...
    def completed_cancelled_tasks(self, tasks):
        ...
        :return:
        '''
        tasks_completed_cancelled = [
            task for task in tasks if task.config['state'] in (u'COMPLETED', u'CANCELLED')]
    ...
    def failed_tasks(self, tasks):
        ...
        '''
        tasks_failed = [
            task for task in tasks if task.config['state'] in (u'FAILED')]
        ...
        for failed in tasks_failed:
            ...
            print('Ingestion of image %s with id %s has failed with message %s' % (
                image_id, task_id, error_message))
...
def main(argv):
    ...
    username_gee = raw_input(
        'Please Enter a Valid GEE User Name (without @gmail.com): ') + "@gmail.com"
    ...
    for active_file in list_of_files_in_process:
        ...
        if "L1" in active_file:
            ...
            if "s1" in active_file or "s2" in active_file:
                metadata = DataManagement.metadata_generator(
                    active_file, 2, seasonal=True)
                metadata_list.append(metadata)
            else:
                metadata = DataManagement.metadata_generator(active_file)
                metadata_list.append(metadata)
        name_image_in_asset = metadata['id_no']
        data_management_session.dataManagement_logger.info(
            'Properties %s ' % str(metadata))
        data_management_session.dataManagement_logger.info(
            'No data %d ' % no_data)
        list_of_images_already_in_asset = data_management_session.get_assets_info(
            name_image_in_asset)
